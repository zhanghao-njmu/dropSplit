...
#' @param score_cutoff A cutoff value of dropSplitScore to determine if a droplet is cell-containing or empty. Default is 0.9, i.g. a droplet with dropSplitScore>0.9 will be classified as \code{Cell} and a with dropSplitScore<0.1  will be classified as \code{Empty}.
...
#' @param Uncertain_downsample Whether use a downsampled Uncertain droplets for predicting.
#' @param Uncertain_downsample_times Number of downsample times for each Uncertain droplet. dropSplitScore for downsampled droplets from the same Uncertain droplet will be averaged.
...
#' @param Cell_rank,Uncertain_rank,Empty_rank Custom nCount_Rank value to mark the droplets as Cell, Uncertain and Empty labels for the data to be trained. Default is automatic. But useful when the default value is considered to be wrong from the MSE plot.
...
#'
#' @return A list of eight objects:
#' \describe{
...
#' \item{plots}{A list of \code{ggplot} object including various droplet QC metrics before and after classification.}
#' }
#'
#' @examples
...
#' @export
dropSplit <- function(counts, score_cutoff = 0.9, GiniThreshold = NULL, Uncertain_downsample = FALSE, Uncertain_downsample_times = 6, modelOpt = FALSE,
                      Cell_rank = NULL, Uncertain_rank = NULL, Empty_rank = NULL,
                      xgb_params = NULL, xgb_nrounds = 20, xgb_early_stopping_rounds = 3, xgb_thread = 8,
                      bounds = list(),
  ...
  if (is.null(Cell_rank)) {
    pks <- find_peaks(
      x = -meta_info$RankMSE[1:inflection],
      left_shoulder = round(0.1 * inflection),
      right_shoulder = inflection
    )
    Cell_rank <- pks[length(pks)] - 1
  }
  if (is.null(Uncertain_rank)) {
    Uncertain_rank <- Cell_rank + which.max(meta_info$RankMSE[Cell_rank:(6 * Cell_rank)]) - 1
  }
  if (is.null(Empty_rank)) {
    Empty_rank <- Uncertain_rank + which.min(meta_info$RankMSE[Uncertain_rank:(6 * Uncertain_rank)]) - 1
  }
  ...
  message(">>> Simulate low depth droplets from pre-defined Cell droplets...")
  if (ncol(Empty_counts) > 100000) {
    Empty_counts <- Empty_counts[, 1:100000]
  }
  i <- sample(x = 1:ncol(Cell_counts), size = , replace = TRUE)
  ...
  if (isTRUE(Uncertain_downsample)) {
    Sim_Cell_nCount_assign <- sample(unique(Matrix::colSums(Empty_counts)), ncol(Sim_Cell_counts), replace = TRUE)
    Sim_Cell_counts <- downsampleMatrix(x = Sim_Cell_counts, prop = Sim_Cell_nCount_assign / Sim_Cell_nCount, bycol = TRUE)

    message(">>> Simulate low depth droplets from pre-defined Uncertain droplets...")
    Sim_Uncertain_counts_list <- list()
    for (i in seq_len(Uncertain_downsample_times)) {
      x <- Uncertain_counts
      colnames(x) <- paste0("Sim", i, "-", colnames(x))
      Sim_Uncertain_nCount <- Matrix::colSums(x)
      Sim_Uncertain_nCount_assign <- sample(unique(Matrix::colSums(Empty_counts)), ncol(x), replace = TRUE)
    ...
    Sim_Cell_nCount_assign <- sample(unique(Matrix::colSums(Uncertain_counts), Matrix::colSums(Empty_counts)), ncol(Sim_Cell_counts), replace = TRUE)
    Sim_Cell_counts <- downsampleMatrix(x = Sim_Cell_counts, prop = Sim_Cell_nCount_assign / Sim_Cell_nCount, bycol = TRUE)

    message(">>> Use raw-depth Uncertain droplets for predicting...")
  ...
  if (isTRUE(Uncertain_downsample)) {
    ...
    dat <- cbind(Matrix::t(norm_counts), dat_other[, !colnames(dat_other) %in% c("CellGini", "GiniScore", "CellEntropy")])
  }
